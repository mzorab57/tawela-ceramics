...
const Services = () => {
  ...
  return (
    <div className=" pt-20  ">
      ...
      <section
        className="bg-neutral-50/50 bg-cover bg-center "
        style={{
          backgroundImage:
        ...
        <div className="grid  md:grid-cols-2 lg:grid-cols-3 ">
          {featuresData.map((feature, index) => (
            <AnimatedComponent
              ...
              key={feature.id}
            >
              <div className="relative lg:space-y-10 space-y-5 overflow-hidden group bg-transparent hover:bg-white lg:border-r lg:p-14 p-5 transition-all duration-500 ease-in-out hover:scale-105 flex flex-col items-start text-start h-full">
                <span className="absolute top-10 text-orange-500/30 right-4 text-xl lg:text-3xl font-jost font-medium ">
                  {`0${feature.id}`}
                </span>
                <div className="mb-6 group-hover:animate-bounce  ">
                  ...
                  <img
                    src={feature.icon}
                    alt={feature.title}
                    className="lg:size-24 size-10  absolute right-16 lg:right-28 top-10 opacity-10 group-hover:hidden"
                  />
                  <span className="lg:size-14 size-12 rounded-full group-hover:hidden bg-orange-500 absolute top-16 left-10 opacity-20"></span>
                </div>
                <h5 className="mb-4 text-2xl font-jost font-medium text-gray-800">
                  {feature.title}
                </h5>
